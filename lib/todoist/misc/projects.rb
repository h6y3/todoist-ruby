  ...
  module Misc
    class Projects < Todoist::Service
        ...
        def get_archived_projects()
          result = @api_helper.get_response(Config::TODOIST_PROJECTS_GET_ARCHIVED_COMMAND)
        ...
        def get_project_info(project, all_data = true)
          result = @api_helper.get_response(Config::TODOIST_PROJECTS_GET_COMMAND, {project_id: project.id, all_data: true})
        ...
        def get_project_data(project)
          result = @api_helper.get_response(Config::TODOIST_PROJECTS_GET_DATA_COMMAND, {project_id: project.id})
          project = result["project"] ? ParseHelper.make_object(result["project"]) : nil
