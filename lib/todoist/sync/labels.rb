module Todoist
  module Sync
    class Labels < Todoist::Service
        ...
        def collection
          return @api_helper.collection("labels")
        end

        # Add a label with a given hash of attributes and returns the label id
        def add(args)
          return @api_helper.add(args, "label_add")
        end

        # Update label given a hash of attributes
        def update(args)
          return @api_helper.command(args, "label_update")
        end

        # Delete a label given a label
        def delete(label)
          args = {id: label.id}
          return @api_helper.command(args, "label_delete")
        end
        ...
        def update_multiple_orders(labels)
          ...
          return @api_helper.command(args, "label_update_orders")
        end
