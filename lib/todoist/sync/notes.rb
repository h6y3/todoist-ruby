module Todoist
  module Sync
    class Notes < Todoist::Service
        ...
        def collection
          return @api_helper.collection("notes")
        end
        ...
        def add(args)
          return @api_helper.add(args, "note_add")
        end

        # Update a note given a hash of attributes
        def update(args)
          return @api_helper.command(args, "note_update")
        end

        # Delete notes given an a note
        def delete(note)
          args = {id: note.id}
          return @api_helper.command(args, "note_delete")
        end
